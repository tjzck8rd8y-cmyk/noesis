...
.nav-link.active { color: var(--text); background: var(--text); color: white; }
.nav-cta {
  padding: 8px 18px;
  background: var(--text);
  color: white;
...
<nav class="nav">
  <div class="container nav-inner">
    ...
    <div class="nav-links">
      <button class="nav-link active" data-page="home" onclick="showPage('home')">Accueil</button>
      <button class="nav-link" data-page="pomodoro" onclick="showPage('pomodoro')">Pomodoro</button>
      <button class="nav-link" data-page="jmethod" onclick="showPage('jmethod')">Méthode des J</button>
      <button class="nav-link" data-page="feynman" onclick="showPage('feynman')">Feynman</button>
      <button class="nav-link" data-page="blankpage" onclick="showPage('blankpage')">Feuille Blanche</button>
      <button class="nav-link" data-page="cornell" onclick="showPage('cornell')">Cornell</button>
      <button class="nav-link" data-page="spaced" onclick="showPage('spaced')">Flashcards</button>
      <button class="nav-link" data-page="subjects" onclick="showPage('subjects')">Sujets</button>
    </div>
    ...
    <div class="container">
      ...
      <div class="cards-grid">
        <div class="card" onclick="showPage('pomodoro')">
          ...
          <div class="card-meta"><span>🇮🇹 Italie</span><span>25 min</span><span>Focus</span></div>
        </div>
        <div class="card" onclick="showPage('jmethod')">
          ...
          <div class="card-meta"><span>🇫🇷 France</span><span>30 jours</span><span>Mémoire</span></div>
        </div>
        <div class="card" onclick="showPage('interleaving')">
          ...
          <div class="card-meta"><span>🌍 International</span><span>Session</span><span>Adaptation</span></div>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="container">
      ...
      <div class="cards-grid">
        <div class="card" onclick="showPage('feynman')">
          ...
          <div class="card-meta"><span>🇺🇸 USA</span><span>45 min</span><span>Compréhension</span></div>
        </div>
        <div class="card" onclick="showPage('blankpage')">
          ...
          <div class="card-meta"><span>🌍 Universel</span><span>20 min</span><span>Rappel</span></div>
        </div>
        <div class="card" onclick="showPage('cornell')">
          ...
          <div class="card-meta"><span>🇺🇸 USA</span><span>Cours</span><span>Organisation</span></div>
        </div>
        <div class="card" onclick="showPage('spaced')">
          ...
          <div class="card-meta"><span>🇩🇪 Allemagne</span><span>Variable</span><span>Long terme</span></div>
        </div>
        <div class="card" onclick="showInfo('sq3r')">
          ...
          <div class="card-meta"><span>🇺🇸 USA</span><span>Lecture</span><span>Compréhension</span></div>
        </div>
        <div class="card" onclick="showInfo('loci')">
          ...
          <div class="card-meta"><span>🇮🇹 Italie</span><span>30 min</span><span>Mémorisation</span></div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="footer">
    <div class="container-narrow">
      ...
      <div class="footer-links">
        <a onclick="showPage('subjects')">Sujets</a>
        <a onclick="showPage('pomodoro')">Pomodoro</a>
        <a onclick="showPage('spaced')">Flashcards</a>
        <a onclick="showPage('feynman')">Feynman</a>
      </div>
    </div>
  </div>
</div>
...
let subjects = loadData('noesis_subjects', JSON.parse(JSON.stringify(defaultSubjects)));
let timerInterval = null, timerSeconds = 25 * 60, timerRunning = false, timerMode = 'pomodoro', cycleCount = 1;
let pomodoroLog = loadData('noesis_pomodoro_log', []);
let jCourses = loadData('noesis_j_courses', []);
let flashcards = loadData('noesis_flashcards', []);
let cornellNotes = loadData('noesis_cornell', []);
let feynmanSessions = loadData('noesis_feynman', []);
...
function addCustomSubject() {
  ...
  subjects.push({ id: 'custom_' + Date.now(), name: name, active: true });
  localStorage.setItem('noesis_subjects', JSON.stringify(subjects));
...
function toggleInterleaveSubject(el, name) {
  ...
  if (el.classList.contains('active')) {
    selectedInterleave.push(name);
  } else {
    selectedInterleave = selectedInterleave.filter(n => n !== name);
  }
}
...
function clearInterleave() {
  ['mon','tue','wed','thu','fri','sat','sun'].forEach(d => {
    const el = document.getElementById(d + '-slots');
    if (el) el.innerHTML = '';
  });
...
function selectPomodoroSubject(name) {
  ...
  const el = document.getElementById('current-subject');
...
function timerComplete() {
  ...
  if (timerMode === 'pomodoro' || timerMode === 'deep') {
    ...
    const goal = document.getElementById('pomodoro-goal');
    const goalVal = goal ? goal.value : '';
    pomodoroLog.push({
      time: new Date().toLocaleTimeString('fr-FR', {hour:'2-digit', minute:'2-digit'}),
      subject: currentPomodoroSubject,
      goal: goalVal,
      duration: timerSettings[timerMode]
    });
    ...
    if (cycleCount % 4 === 0) {
      ...
      if (modes[2]) modes[2].classList.add('active');
    } else {
      setTimerMode('short');
      const modes = document.querySelectorAll('.timer-mode');
      if (modes[1]) modes[1].classList.add('active');
    }
  } else {
    showToast('Pause terminée ! Retour au travail.');
    setTimerMode('pomodoro');
    const modes = document.querySelectorAll('.timer-mode');
    if (modes[0]) modes[0].classList.add('active');
  }
  const display = document.getElementById('timer-display');
  if (display) {
    display.classList.add('pulse');
    setTimeout(function() { display.classList.remove('pulse'); }, 2000);
  }
}
...
function renderPomodoroLog() {
  ...
  container.innerHTML = pomodoroLog.slice(-10).reverse().map(function(log) {
    return '<div class="log-entry"><span class="log-time">' + log.time + '</span><span class="log-dot" style="background: var(--text)"></span><span class="log-content">' + log.subject + (log.goal ? ' — ' + log.goal : '') + '</span><span class="log-duration">' + log.duration + 'min</span></div>';
  }).join('');
}
...
function showJInfo(day) {
  ...
  const el = document.getElementById('j-info');
  if (el) el.innerHTML = info[day] || info[0];
  document.querySelectorAll('.j-node').forEach(function(n) {
    n.classList.remove('current');
    if (n.textContent.trim() === 'J' + day) n.classList.add('current');
  });
}
...
function addJCourse() {
  ...
  const revisions = rhythms[rhythm].map(function(d) {
    const rev = new Date(j0);
    rev.setDate(rev.getDate() + d);
    return { day: d, date: rev.toISOString().split('T')[0] };
  });
  jCourses.push({ title: title, subject: subject, date: date, rhythm: rhythm, revisions: revisions });
...
function renderJUpcoming() {
  ...
  let upcoming = [];
  jCourses.forEach(function(c) {
    c.revisions.forEach(function(r) {
      if (r.date >= today) upcoming.push({ day: r.day, date: r.date, title: c.title, subject: c.subject });
    });
  });
  ...
  container.innerHTML = upcoming.slice(0, 10).map(function(u) {
...
function setFeynmanStep(step) {
  feynmanStep = step;
  document.querySelectorAll('.feynman-step').forEach(function(s, i) {
  ...
  if (card && c) card.innerHTML = '<h2>' + c.title + '</h2><p class="subtitle">' + c.subtitle + '</p>' + c.html;
}

function nextFeynmanStep() {
  if (feynmanStep < 4) setFeynmanStep(feynmanStep + 1);
}

function saveFeynmanSession() {
  const conceptEl = document.getElementById('feynman-concept');
  const concept = conceptEl ? conceptEl.value || 'Concept sans titre' : 'Concept sans titre';
  feynmanSessions.push({ concept: concept, date: new Date().toLocaleDateString('fr-FR'), completed: true });
...
function renderFeynmanSessions() {
  ...
  container.innerHTML = feynmanSessions.slice(-5).reverse().map(function(s) {
    return '<div class="checklist-item"><div class="check-box checked">✓</div><span>' + s.concept + ' <span style="color: var(--text-muted);">— ' + s.date + '</span></span></div>';
  }).join('');
}
...
function startBlankPage() {
  ...
  const session = document.getElementById('bp-session');
  if (session) session.style.display = 'block';
  bpSeconds = time * 60;
  const ta = document.getElementById('bp-textarea');
  if (ta) { ta.value = ''; ta.focus(); }
  bpTimerInterval = setInterval(function() {
    if (bpSeconds > 0) {
      bpSeconds--;
      const m = Math.floor(bpSeconds / 60);
      const s = bpSeconds % 60;
      const display = document.getElementById('bp-timer-display');
      if (display) display.textContent = 'Temps restant: ' + m + ':' + s.toString().padStart(2, '0');
      const total = time * 60;
...
function saveCornell() {
  const note = {
    title: document.getElementById('cornell-title').value || 'Sans titre',
    cues: document.getElementById('cornell-cues').value,
    notes: document.getElementById('cornell-notes').value,
    summary: document.getElementById('cornell-summary').value,
...
function renderCornellSaved() {
  ...
  container.innerHTML = cornellNotes.slice(-5).reverse().map(function(n) {
    return '<div class="sr-list-item" style="cursor: pointer;" onclick="loadCornell(\'' + n.date + '\')"><div class="sr-info"><h4>' + n.title + '</h4><p>' + n.date + ' • ' + n.notes.length + ' caractères</p></div></div>';
  }).join('');
}

function loadCornell(date) {
  const note = cornellNotes.find(function(n) { return n.date === date; });
  if (!note) return;
...
function renderSrList() {
  ...
  if (!container) return;
  const now = new Date();
  const due = flashcards.filter(function(f) { return new Date(f.nextReview) <= now; });
  const upcoming = flashcards.filter(function(f) { return new Date(f.nextReview) > now; });
  const countEl = document.getElementById('sr-count');
  if (countEl) countEl.textContent = due.length + ' cartes en attente';
  ...
  let html = due.slice(0, 5).map(function(f) {
    return '<div class="sr-list-item" style="cursor: pointer;" onclick="startSrReview(' + f.id + ')"><div class="sr-info"><h4>' + f.front.substring(0, 50) + (f.front.length > 50 ? '...' : '') + '</h4><p>' + f.subject + ' • ' + f.reviews + ' révisions</p></div><span class="sr-badge sr-now">À RÉVISER</span></div>';
  }).join('');
  html += upcoming.slice(0, 3).map(function(f) {
...
function startSrReview(id) {
  const card = flashcards.find(function(f) { return f.id === id; });
  if (!card) return;
  currentReviewCard = card;
  const review = document.getElementById('sr-review');
  if (review) review.style.display = 'block';
  const front = document.getElementById('sr-review-front');
  if (front) front.textContent = card.front;
  const back = document.getElementById('sr-review-back');
  if (back) { back.style.display = 'none'; back.textContent = ''; }
  const showBtn = document.getElementById('sr-show-btn');
  if (showBtn) showBtn.style.display = 'inline-flex';
  const rating = document.getElementById('sr-rating');
...
function showSrAnswer() {
  const back = document.getElementById('sr-review-back');
  if (back) { back.textContent = currentReviewCard.back; back.style.display = 'block'; }
  const showBtn = document.getElementById('sr-show-btn');
...
function rateCard(rating) {
  ...
  const card = flashcards.find(function(f) { return f.id === currentReviewCard.id; });
  if (!card) return;
...
function showInfo(method) {
  ...
  if (bodyEl) bodyEl.textContent = i.body;
...
document.addEventListener('keydown', function(e) {
  if (e.code === 'Space') {
    ...
    if (pomodoro && pomodoro.classList.contains('active')) {
      e.preventDefault();
      toggleTimer();
    }
  }
